...
const requestPayoutSchema = z.object({
  amountCents: z.number().int().positive().optional(), //выведем весь доступный баланс
  currency: z.string().min(3).max(10).default("CZK"),
  destination: z.any().optional() // сделать позже: iban/bic/stripeAccountId
});
...
//process payout (mark PAID / REJECTED / FAILED + refund)
