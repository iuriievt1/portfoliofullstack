...
DL_DOC=$(curl -s -X POST "$BASE/driver/documents" \
  ...
  -d '{"type":"DRIVER_LICENSE","number":"DL-CARD-1","files":[{"name":"dl.jpg"}]}')
echo "$DL_DOC" | pp
...
VR_DOC=$(curl -s -X POST "$BASE/driver/documents" \
  -H "Authorization: Bearer $DRIVER_TOKEN" -H "Content-Type: application/json" \
  -d '{"type":"VEHICLE_REG","number":"REG-CARD-1","files":[{"name":"reg.jpg"}]}')
echo "$VR_DOC" | pp
...
echo "== passenger create ride (CARD) =="
RIDE_CREATE=$(curl -s -X POST "$BASE/passenger/rides" \
  -H "Authorization: Bearer $PASS_TOKEN" -H "Content-Type: application/json" \
  -d "{\"pickupAddress\":\"Test Pickup\",\"pickupLat\":50.087,\"pickupLng\":14.421,\"destinationAddress\":\"Test Dest\",\"destinationLat\":50.087,\"destinationLng\":14.421,\"tariffId\":\"$TARIFF_ID\",\"paymentMethod\":\"CARD\"}")
echo "$RIDE_CREATE" | pp
...
COMPLETE=$(curl -s -X POST "$BASE/rides/$RIDE_ID/status" \
  -H "Authorization: Bearer $DRIVER_TOKEN" -H "Content-Type: application/json" \
  -d '{"status":"COMPLETED"}')
echo "$COMPLETE" | pp
echo
...
echo "== driver wallet summary (should include FARE, no duplicates) =="
curl -s -H "Authorization: Bearer $DRIVER_TOKEN" "$BASE/wallet/summary" | pp
echo

echo "== passenger cashDebtCents in DB (should stay 0) =="
docker compose exec -T postgres psql -U vezi -d vezi -c \
"select \"cashDebtCents\" from \"PassengerProfile\" where \"userId\"='$PASS_ID';"
echo

echo "== ridePayment in DB (should be CAPTURED for CARD) =="
docker compose exec -T postgres psql -U vezi -d vezi -c \
"select \"rideId\", method, status, \"amountCents\", currency from \"RidePayment\" where \"rideId\"='$RIDE_ID';"
echo

echo "== ride flags in DB (cashDebtApplied should be false for CARD) =="
docker compose exec -T postgres psql -U vezi -d vezi -c \
"select id, \"paymentMethod\", \"cashDebtApplied\" from \"Ride\" where id='$RIDE_ID';"
echo
