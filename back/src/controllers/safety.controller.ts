...
function buildShareToken() {
  // короткий безопасный токен (подходит для URL)
...
const shareStopSchema = z.object({
  rideId: z.string().min(5),
  token: z.string().min(10).optional() // можно передать конкретный токен
});
...
export const stopShareTripHandler = asyncHandler(async (req, res) => {
  ...
  // если token не передали — остановим последний SHARE_STARTED этого пользователя для этой поездки
...
const sosSchema = z.object({
  ...
  // поддержим и message и note (чтобы не ломать клиентов)
...
export const publicShareViewHandler = asyncHandler(async (req, res) => {
  ...
  // найдем старт по токену (JSON path filter)
  ...
  // публично отдаём МИНИМУМ: без телефона пассажира и т.п.
  const ride = await prisma.ride.findUnique({
