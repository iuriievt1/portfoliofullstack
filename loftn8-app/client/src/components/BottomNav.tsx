...
function Icon({
  ...
  const common = { className: cls, width: 22, height: 22, fill: "none", strokeWidth: 1.8, strokeLinecap: "round" as const, strokeLinejoin: "round" as const };
  if (name === "menu")
    return (
      ...
      <div className="mx-auto max-w-md px-4">
        <div className="rounded-[26px] border border-white/10 bg-white/5 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.45)]">
          <div className="grid grid-cols-4 px-1">
            <Item href="/menu" label="Меню" icon="menu" />
            <Item href="/cart" label="Корзина" icon="cart" badge={count || undefined} />
            <Item href="/call" label="Персонал" icon="call" />
            <Item href="/profile" label="Профиль" icon="profile" />
          </div>
        </div>
      </div>
    </div>
