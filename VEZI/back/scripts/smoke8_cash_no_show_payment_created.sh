...
DL_DOC="$(curl -s -X POST "$BASE/driver/documents" \
  ...
  -d '{"type":"DRIVER_LICENSE","number":"DL-CASH-NOSHOW","files":[{"name":"dl.jpg"}]}')"
...
VR_DOC="$(curl -s -X POST "$BASE/driver/documents" \
  -H "Authorization: Bearer $DRIVER_TOKEN" -H "Content-Type: application/json" \
  -d '{"type":"VEHICLE_REG","number":"REG-CASH-NOSHOW","files":[{"name":"reg.jpg"}]}')"
...
"insert into \"Vehicle\"(id,\"driverId\",plate,\"brandModel\",color,category)
 values ('$VID','$DRIVER_PROFILE_ID','2CD${SUF}','Skoda Superb','Black','business');"
fi
echo "vehicle ok"
echo
...
echo "== driver NO-SHOW (CASH -> debt grows; payment status CREATED kind=NO_SHOW_FEE) =="
...
echo "== ridePayment in DB (should exist, CASH, status=CREATED, kind=NO_SHOW_FEE) =="
docker compose exec -T postgres psql -U vezi -d vezi -c \
