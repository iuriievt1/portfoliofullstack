...
echo "== driver wallet summary (after no-show) =="
curl -s -H "Authorization: Bearer $DRIVER_TOKEN" "$BASE/wallet/summary" | pp
echo

echo "== payout request #1 (1000) =="
P1=$(curl -s -X POST "$BASE/wallet/payouts/request" \
  -H "Authorization: Bearer $DRIVER_TOKEN" -H "Content-Type: application/json" \
  -d '{"amountCents":1000,"currency":"CZK","destination":{"iban":"TEST_IBAN"}}')
echo "$P1" | pp
P1_ID=$(echo "$P1" | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d["payout"]["id"])')
echo "P1_ID=$P1_ID"

echo "== internal process #1 -> PAID =="
curl -s -X POST "$BASE/wallet/payouts/$P1_ID/process" \
  -H "x-internal-key: $INTERNAL_KEY" -H "Content-Type: application/json" \
  -d '{"action":"PAID","note":"smoke paid"}' | pp
echo

echo "== driver wallet summary (after PAID) =="
curl -s -H "Authorization: Bearer $DRIVER_TOKEN" "$BASE/wallet/summary" | pp
echo

echo "== payout request #2 (1000) =="
P2=$(curl -s -X POST "$BASE/wallet/payouts/request" \
  -H "Authorization: Bearer $DRIVER_TOKEN" -H "Content-Type: application/json" \
  -d '{"amountCents":1000,"currency":"CZK","destination":{"iban":"TEST_IBAN"}}')
echo "$P2" | pp

# если вдруг ok:false — выходим красиво
OK2=$(echo "$P2" | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d.get("ok", False))')
if [[ "$OK2" != "True" ]]; then
  echo "Payout #2 request failed above (see error)."; exit 1
fi

P2_ID=$(echo "$P2" | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d["payout"]["id"])')
echo "P2_ID=$P2_ID"

echo "== internal process #2 -> REJECTED (refund) =="
...
echo "== driver wallet summary after REJECTED (should refund) =="
curl -s -H "Authorization: Bearer $DRIVER_TOKEN" "$BASE/wallet/summary" | pp
echo

echo "== passenger cashDebtCents in DB =="
